...
function buildImagenSpriteSheetPrompt(basePrompt: string, situationLabel: string) {
  return [
    ...
    "Use a pure white background (#FFFFFF) only across the entire image. No transparency.",
    "No room, no street, no scenery, no props, and no furniture in the background.",
    "Absolutely no borders, no frames, no background boxes, no panels, no ground shadows, and no background plates.",
    "Do not add any numbers, captions, subtitles, or watermarks.",
    "Use a clean 4x4 grid layout. Each cell must contain the same character centered in a square area with zero extra background elements.",
